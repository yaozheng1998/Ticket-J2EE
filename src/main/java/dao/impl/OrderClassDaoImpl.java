...
public class OrderClassDaoImpl implements OrderClassDao {
    ...
    public List<OrderClass> getClassFromVIPName(String name) {
        String sql="select oc.orderclass_id,oc.order_id,oc.class_id,oc.real_name,oc.phone,oc.grade,oc.state from `order_classes` oc,`order` o where o.order_id=oc.order_id and o.vip_name="+name;
    ...
    public List<OrderClass> getClassByState(String name,String state) {
        String sql="select oc.orderclass_id,oc.order_id,oc.class_id,oc.real_name,oc.phone,oc.grade,oc.state from `order_classes` oc,`order` o where o.order_id=oc.order_id and o.vip_name="+name+" and oc.state="+state;
        List<Object[]> classes=baseDao.querySQL(sql);
        return this.getOrderClasses(classes);
    }
    ...
    private List<OrderClass> getOrderClasses(List<Object[]> objects){
        ...
        for(Object[] object:objects){
            ...
            orderClass.setOrder_id((Integer)object[1]);
